...
export function initSakuraEffects(root=document){
  ...
  if(canvas&&canvas.getContext){
    ...
    function targetCount(){const base=coarse?Math.round(w/120):Math.round(w/92);return clamp(Math.round(base*intensity),coarse?6:8,coarse?14:20)}
    function seed(){petals=Array.from({length:targetCount()},()=>makePetal(true))}
    function resize(){dpr=Math.min(devicePixelRatio||1,2);w=innerWidth;h=innerHeight;canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);canvas.style.width=`${w}px`;canvas.style.height=`${h}px`;ctx.setTransform(dpr,0,0,dpr,0,0);seed()}
    ...
    function tick(t){if(paused)return;ctx.clearRect(0,0,w,h);petals.forEach((p,i)=>{p.y+=p.speed;p.x+=Math.sin(t*p.wobble+p.phase)*p.drift+p.side;p.rot+=p.spin;if(p.y>h+50||p.x<-70||p.x>w+70)petals[i]=makePetal(false);draw(p)});raf=requestAnimationFrame(tick)}
    const onResize=()=>resize();window.addEventListener('resize',onResize);resize();if(!reduced)raf=requestAnimationFrame(tick);
    petalController={setIntensity(value){intensity=clamp(Number(value)||1,.35,1.7);seed()},pause(){paused=true;cancelAnimationFrame(raf)},resume(){if(reduced||!paused)return;paused=false;raf=requestAnimationFrame(tick)},destroy(){cancelAnimationFrame(raf);window.removeEventListener('resize',onResize);ctx.clearRect(0,0,w,h)}};
